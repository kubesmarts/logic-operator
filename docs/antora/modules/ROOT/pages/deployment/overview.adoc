...
Logic Operator is a Kubernetes operator that manages Serverless Logic workflows powered by Quarkus Flow.
...
|`LogicPlatform`
|Platform-wide configuration and shared services

|`LogicFlowRuntime`
|Manages the Quarkus Flow Runner pod(s) and their configuration

|`LogicFlowDefinition`
|Stores a Serverless Workflow DSL definition and links it to a runtime
